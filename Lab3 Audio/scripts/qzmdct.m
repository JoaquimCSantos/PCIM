...
%----------------- quantization area ------------------------------%
    if (white==1)
        % add code here
        Ps              = sum(abs(mdct(freqregion)).^2);
        delta           = sqrt((12*Ps)/(N2*10^(SMR/10)));
        Xq              = floor(0.5 + (mdct(freqregion) ./ delta));  
        mdct(freqregion)=  delta .* Xq;
    else
        
        for k = freqregion
            Ps(k)       = abs(mdct(k))^2;
            delta(k)    = sqrt((12*Ps(k))/(10^(SMR/10)));
            Xq(k)       = floor(0.5 +(mdct(k)/delta(k)));
            mdct(k)     = delta(k)*Xq(k);
        end
...
% IMDCT 2 IODFT
%
    fdata(1:N2)= 2 * (mdct(1:N2).*cosargmdct + i*mdct(1:N2).*sinargmdct);
...
figure;
fftdatain = fft(data1 , N);
fftdifsignal = fft(difsignal , N);
plot(FS/N*(freqregion -1), 20*log10(abs(fftdatain(freqregion))));
hold on;
plot(FS/N*(freqregion -1), 20*log10(abs(fftdifsignal(freqregion))));
hold off;

xlabel('Frequência (Hz)');
ylabel('Densidade Espetral (dB)');
title('Espetro Sinal e MDCT com Ruído Colorido SNR = 5 DB');
